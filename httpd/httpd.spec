...
BuildRequires: apr-devel >= 1.4.0, apr-util-devel >= 1.4.0, pcre-devel >= 5.0, 	distcache-devel
Requires: initscripts >= 8.36, /etc/mime.types, apr >= 1.4.0, distcache, mod_authnz_ldap >= %{version}, mod_lua >= %{version}, mod_socache_dc >= %{version}, mod_ssl >= %{version}
...
%configure \
	...
	--with-suexec-uidmin=500 --with-suexec-gidmin=100 \
        ...
        --enable-ssl --with-ssl --enable-socache-dc --enable-socache-shmcb --enable-bucketeer \
	...
	--enable-proxy  --enable-proxy-fdpass --enable-proxy-http --enable-proxy-ajp --enable-proxy-balancer --enable-proxy-hcheck \
	--enable-authnz-ldap \
	--enable-modules=all \
        --disable-imagemap
...
# Enable some modules
sed -i "s/#LoadModule proxy_hcheck_module/LoadModule proxy_hcheck_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule watchdog_module/LoadModule watchdog_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule slotmem_shm_module/LoadModule slotmem_shm_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule rewrite_module/LoadModule rewrite_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule lbmethod_byrequests_module/LoadModule lbmethod_byrequests_module /g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule lbmethod_byrequests_module/LoadModule lbmethod_byrequests_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule lbmethod_bytraffic_module/LoadModule lbmethod_bytraffic_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule lbmethod_bybusyness_module/LoadModule lbmethod_bybusyness_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule lbmethod_heartbeat_module/LoadModule lbmethod_heartbeat_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule proxy_ajp_module/LoadModule proxy_ajp_module /g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule proxy_module/LoadModule proxy_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf 
sed -i "s/#LoadModule proxy_balancer_module/LoadModule proxy_balancer_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule proxy_http_module/LoadModule proxy_http_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/#LoadModule expires_module/LoadModule expires_module/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
 
#Include conf.d
echo -e "#Include conf.d directory 
IncludeOptional %{_sysconfdir}/httpd/conf.d/*.conf" >> $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf

#Change user and group
sed -i "s/User daemon/User apache/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf
sed -i "s/Group daemon/Group apache/g" $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/httpd.conf

cp $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf/extra/*.conf $RPM_BUILD_ROOT%{_sysconfdir}/httpd/conf.modules.d/
